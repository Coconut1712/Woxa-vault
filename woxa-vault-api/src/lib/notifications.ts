...
//     * No secret/plaintext is ever stored; `metadata` is documented as
//       roles/kinds/counts/booleans only.
 ...
 *
 * Returns true when a row was written, false when skipped (self-action).
 */
export async function createNotification(tx: Tx, input: CreateNotificationInput): Promise<boolean> {
  ...
  // (A user sharing with themselves, or — defensively — any path where the
  // computed recipient equals the actor.)
