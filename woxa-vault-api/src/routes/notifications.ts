...
export const notificationRoutes = new Hono<{ Variables: AuthVariables }>()
  ...
  // ------------------------------------------------------------------
  // GET /notifications?limit= — the caller's own notifications, newest first,
  // plus their unread badge count.
  // ------------------------------------------------------------------
  ...
  .get("/unread-count", async (c) => {
    const user = c.get("user")!;
    return c.json({ unreadCount: await unreadCountFor(user.id) });
  })

  // ------------------------------------------------------------------
  // POST /notifications/read-all — mark all the caller's unread as read.
  // Registered BEFORE /:id/read (static-before-param).
  // ------------------------------------------------------------------
  ...
  .post("/:id/read", paramValidator(idParam), async (c) => {
    ...
    // The owner pin is BOTH the authorization check and the existence check:
    // a row that exists but belongs to another user returns 0 updated rows,
    ...
    if (updated.length === 0) {
      // Could be: id doesn't exist, OR it's someone else's. Either way 404 so
      // ownership can't be probed. (Idempotency for an already-read OWN row is
      // preserved because the UPDATE still matches it — it returns 1 row even
      // when read_at was already set.)
